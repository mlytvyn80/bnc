...
class t_eph {
 public:
  ...
  t_irc   getCrd(const bncTime& tt, NEWMAT::ColumnVector& xc, NEWMAT::ColumnVector& vv, bool useCorr) const;
...
class t_ephGlo : public t_eph {
 ...
 private:
  ...
  mutable NEWMAT::ColumnVector _xv;  // status vector (position, velocity) at time _tt
